...
class Backend extends CI_Controller {
	...
	public function __construct() {
		...
		$this->path_upload = FCPATH.'uploads/dashboard/';
	...
	public function index(){
		$data['row'] = $this->model_dashboard->get_data_single();
		...
		$this->load->view('dashboard/dashboard/form', $data);
	}
	
	public function save_update($id = ''){	
		
		$this->form_validation->set_rules('dashboard_detail_th', 'Detail(Th)', "trim|required");
		$this->form_validation->set_rules('dashboard_detail_en', 'Detail(En)', "trim|required");
		
		if($this->form_validation->run($this) == TRUE) {
			$data = array(
				'dashboard_detail_th' => $this->input->post('dashboard_detail_th'),
				'dashboard_detail_en' =>  $this->input->post('dashboard_detail_en'),
				'dashboard_username_update' => $this->session->userdata('session_username'),
				'dashboard_datetime_update' => date('Y-m-d H:i:s'),
				'dashboard_ip_update' => $_SERVER['REMOTE_ADDR']
			);
				
			$this->model_dashboard->update_data($data, $id);
			
			redirect('dashboard/backend/index');
		} else {
			$this->index($id);
		}
	}
