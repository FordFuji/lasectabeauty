...
class Backend extends CI_Controller {
	...
	public function server_processing() {
        ...
        foreach ($list as $contact) {
            ...
            $row[] = $contact->contact_full_name;
            $row[] = $contact->contact_phone_number;
            $row[] = $contact->contact_email;
            $row[] = $contact->contact_subject;
            $row[] = $contact->contact_message;
            $row[] = $contact->contact_datetime_create;
 			$row[] = $contact->contact_ip_create;

 			$row[] = '<a href="'.site_url('contact/backend/delete/'.$contact->contact_id).'" onclick="return confirm(\'Confirm Delete\');">Delete</a>';
	...
	public function delete($contact_id) {
		...
		$this->db->delete('ci_contact', $where);
