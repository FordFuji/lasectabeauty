...
if(!empty($backCtrl)) {
?>
							<div style="float:left; width:30%;"><input type="button" value="Back" onclick="window.location.href='<?php echo site_url('order/backend/all_form/'.$backCtrl->order_detail_id);?>';"></div>
<?php
}
?>  
...
if(!empty($nextCtrl)) {
?>
							<div style="float:right; width:30%; text-align: right;"><input type="button" value="Next" onclick="window.location.href='<?php echo site_url('order/backend/all_form/'.$nextCtrl->order_detail_id);?>';"></div>
<?php
}
?>                                         
                        	<p>&nbsp;</p>
...
if(!empty($row)) {
?>                      
                        	<!-- <div align="right"><input type="button" value="Print" onclick="window.open('<?php echo site_url('order/backend/printOrder/'.$row->order_detail_id);?>');"></div> -->
...
?>                  
					        ...
					        <div class="form-group">
					            ...
					            <div class="col-md-9">
                       				<select name="order_detail_status" id="order_detail_status" class="form-control" onchange="changeStatus(this.value);">
...
<?php
if(!empty($row) and $row->order_detail_status == 'Shipping') {
	$display = 'block';
} else {
	$display = 'none';
}
?>					   
					        ...
					        <div class="form-group" id="tracking_no" style="display: <?php echo $display;?>;">
					            <label class="col-md-3 control-label" align="right">Tracking No.</label>
					            <div class="col-md-9">
                       				<input type="text" name="order_detail_tracking_no" id="order_detail_tracking_no" value="<?php if(!empty($row)) echo $row->order_detail_tracking_no;?>" class="form-control">             
					            </div>
					        </div>
					        <br>
					        <br>
					        <div class="form-group">
					            <label class="col-md-3 control-label" align="right">&nbsp;</label>
					            <div class="col-md-9">                       
...
?>
	...
	<script>
		...
		function changeStatus(status) {
			if($("#order_detail_status").val() == 'Shipping' || $("#order_detail_status").val() == 'Success') {
				$("#tracking_no").show();
			} else 
{				$("#tracking_no").hide();
			}
